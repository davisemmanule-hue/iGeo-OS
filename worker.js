...
async function handleOpportunityCollection(request,env){
  ...
  collectionAttempts.set(client,now);const url=new URL(request.url),days=Math.min(Math.max(Number(url.searchParams.get('days')||30),1),90),body=request.method==='POST'?await request.json().catch(()=>({})): {},profiles=normalizeSearchProfiles(body.searchProfiles),sources=[],opportunities=[],startedAt=new Date().toISOString();
  ...
  return json({ok:true,collectedAt:new Date().toISOString(),summary:{discoveredRecords:opportunities.length,matchedRecords:matched.length,filteredRecords:opportunities.length-matched.length,addedRecords:matched.length,changedRecords:0,expiredRecords:0,errors:sources.filter(x=>x.errorState).length},sources,opportunities:matched});
}
