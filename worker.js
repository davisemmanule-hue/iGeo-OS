...
async function handleOpportunityCollection(request,env){
  ...
  collectionAttempts.set(client,now);const collectionStarted=Date.now(),url=new URL(request.url),days=Math.min(Math.max(Number(url.searchParams.get('days')||30),1),90),body=request.method==='POST'?await request.json().catch(()=>({})): {},profiles=normalizeSearchProfiles(body.searchProfiles),sources=[],opportunities=[],startedAt=new Date(collectionStarted).toISOString();
  for(const source of OPPORTUNITY_SOURCES){if(!source.enabled||source.collectionMethod==='Manual'){sources.push({...sourceView(source,env),lastAttemptedCheck:source.collectionMethod==='Manual'?null:startedAt,durationMs:0,resultCount:0});continue}const adapter=SOURCE_ADAPTERS[source.id];if(!adapter){sources.push({...sourceView(source,env),connectionStatus:'Unsupported',lastAttemptedCheck:startedAt,durationMs:0,resultCount:0});continue}const sourceStarted=Date.now();try{const result=await adapter.collect({env,days}),durationMs=Date.now()-sourceStarted;opportunities.push(...result.records);sources.push({...sourceView(source,env),connectionStatus:result.status,lastAttemptedCheck:startedAt,lastSuccessfulCheck:result.status==='Connected'?new Date().toISOString():null,recordCount:result.records.length,resultCount:result.records.length,sourceCount:Number(result.totalRecords||result.records.length),durationMs,errorState:''})}catch(error){sources.push({...sourceView(source,env),connectionStatus:'Temporarily Unavailable',lastAttemptedCheck:startedAt,durationMs:Date.now()-sourceStarted,resultCount:0,errorState:error?.message||'Collection unavailable; try again later.'})}}
...
function mapSamOpportunity(item) { const contact=item.pointOfContact?.[0]||{},collectedAt=new Date().toISOString(); return { id:`sam-${item.noticeId||item.solicitationNumber}`, externalSourceId:item.noticeId||item.solicitationNumber||'', origin:'automated', source:'SAM.gov', procurementSource:'SAM.gov', sourceUrl:item.uiLink||item.resourceLinks?.[0]||'', opportunityUrl:item.uiLink||item.resourceLinks?.[0]||'', solicitationNumber:item.solicitationNumber||item.noticeId||'', title:item.title||'Title not published', buyer:item.fullParentPathName||item.department||'', agency:item.organizationName||item.subTier||'', solicitationType:item.type||item.baseType||'', contractType:item.type||item.baseType||'', postedDate:item.postedDate||'', dueDate:item.responseDeadLine||item.archiveDate||'', naics:item.naicsCode||'', psc:item.classificationCode||'', placeOfPerformance:[item.placeOfPerformance?.city?.name,item.placeOfPerformance?.state?.code].filter(Boolean).join(', '), location:[item.placeOfPerformance?.city?.name,item.placeOfPerformance?.state?.code].filter(Boolean).join(', '), setAside:item.typeOfSetAsideDescription||item.typeOfSetAside||'', estimatedValue:item.award?.amount||'', contactName:contact.fullName||'', contactEmail:contact.email||'', contactPhone:contact.phone||'', attachments:item.resourceLinks||[], scopeSummary:item.description||'', sourceTimestamp:item.modifiedDate||item.postedDate||'', dateCollected:collectedAt,collectedAt,status:'NEW',stage:'READY FOR ANALYSIS',analysisStatus:'PENDING',verificationStatus:'Unverified Lead' }; }
